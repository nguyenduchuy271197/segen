...
export function CreateSeriesDialog() {
  ...
  useEffect(() => {
    if (progress?.step === "Đang tạo các bài học..." && progress.data?.episodes) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const eventSource = new EventSource(`/api/ai/generate-series?topic=${encodeURIComponent(topic)}`);
        ...
        try {
          ...
          if (seriesId) {
            ...
            toast({
              ...
              description: "Không thể tìm thấy ID series. Vui lòng kiểm tra danh sách series của bạn.",
              variant: "destructive",
            });
          }
        } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="bg-card border rounded-lg p-4">
                <h3 className="font-semibold text-lg mb-2">{progress.data.title}</h3>
                {progress.data.description && (
                  <p className="text-sm text-muted-foreground mb-3">{progress.data.description}</p>
                )}
                      ...
                      <div 
                        ...
                        className={cn(
                          ...
                          i === activeEpisodeIndex 
                            ...
                            : "bg-card",
                          progress.step === "Hoàn thành" && "bg-card hover:bg-muted/50"
                        )}
                      >
                        <div className="p-3">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center">
                              <div className="bg-muted w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0">
                                <span className="text-xs font-medium">{episode.order}</span>
                              </div>
                              <h5 className="font-medium text-sm ml-2">{episode.title}</h5>
                            </div>
                            
                            {(i < activeEpisodeIndex || progress.step === "Hoàn thành") && (
                              <Check className="h-4 w-4 text-green-500" />
                            )}
                            
                            {i === activeEpisodeIndex && progress.step === "Đang tạo các bài học..." && (
